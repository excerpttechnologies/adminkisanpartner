...
export default function DistrictsPage() {
  ...
  const fetchStates = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Set first state as default filter if states exist
        if (response.data.data.length > 0) {
          setFilterStateId(response.data.data[0]._id);
          setSelectedStateId(response.data.data[0]._id);
        }
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    if (states.length > 0 && filterStateId) {
      fetchDistricts(1, "", filterStateId);
    }
  }, [states, filterStateId]);
  ...
  const openAdd = () => {
    setEditId(null);
    setDistrictName("");
    setSelectedStateId(filterStateId || (states.length > 0 ? states[0]._id : ""));
    setShowModal(true);
  };
            ...
            <>
                    ...
                    {districts.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="border px-3 py-8 text-center text-gray-500">
                          {search 
                            ? `No districts found matching "${search}" in ${getStateNameById(filterStateId)}`
              ...
              {districts.length > 0 && (
                <div className="text-sm text-gray-600 mt-3 text-center">
                  ...
                  {pagination.total} districts in {getStateNameById(filterStateId)}
                </div>
              )}
            </>
          ) : (
            <div className="text-center py-8 bg-gray-50 rounded">
              <div className="text-gray-500 mb-3">Please select a state to view districts</div>
            </div>
          )}
        </>
      )}
