...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:static z-40 h-screen w-64 bg-white border-r shadow
        transition-transform duration-300 flex flex-col
        ${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        {/* Optional Header */}
        <div className="h-16 flex items-center px-5 font-bold text-green-600 border-b">
          AgroAdmin
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-4">
          {menu.map((group, i) => (
            <div key={i} className={`${i !== 0 && "border-t mt-4 pt-4"}`}>
