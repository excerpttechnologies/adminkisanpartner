...
const Todo = () => {
  ...
  const [list,setList]=useState<{_id:string,todo:string}[]>([])
  ...
  const deleteTodo=async(id:string)=>{
     const res=await axios.del(`http://localhost:3000/api/todo/${id}`)
     setChage(res.data)
  }
 ...
 {/* /////form */}
      ...
      <div className="flex flex-col gap-y-3 p-3 w-96 ">
        ...
        {
         list.map((data,i)=>(
          <div key={i} className="bg-white border flex justify-between border-zinc-300 rounded p-2">
            <p>{data.todo}</p>
            <div className="w-28 flex *:w-12 *:h-10 *:text-white *:flex *:justify-center *:items-center justify-between items-center">
             <button className="bg-green-500">E</button><button onClick={deleteTodo(data._id)} className="bg-red-500">X</button>
            </div>
          </div>
         ))
        }
