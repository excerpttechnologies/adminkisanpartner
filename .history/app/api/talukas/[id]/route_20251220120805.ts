...
export async function PUT(
  ...
  try {
    ...
    const duplicateTaluka = await Taluka.findOne({
      _id: { $ne: id },
      name: name.trim(),
      districtId: district // Use districtId
    });
    if (duplicateTaluka) {
      return NextResponse.json(
    ...
    // Update with districtId
    const updatedTaluka = await Taluka.findByIdAndUpdate(
      id,
      { name: name.trim(), districtId: district }, // Use districtId
      { new: true }
    )
    .populate({
      path: 'districtId',
    ...
    return NextResponse.json({
      ...
      data: {
        _id: updatedTaluka?._id,
        name: updatedTaluka?.name,
        district: updatedTaluka?.districtId,
        districtName: (updatedTaluka?.districtId as any)?.name || 'Unknown District',
        stateName: (updatedTaluka?.districtId as any)?.state?.name || 'Unknown State'
      }
    });
  } catch (error: any) {
