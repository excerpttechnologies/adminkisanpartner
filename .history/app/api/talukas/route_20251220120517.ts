...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingTaluka = await Taluka.findOne({ 
      name: name.trim(), 
      districtId: district // CHANGE HERE: Use districtId
    });
    if (existingTaluka) {
      return NextResponse.json(
    ...
    // CREATE with districtId instead of district
    ...
    const populatedTaluka = await Taluka.findById(taluka._id)
      .populate('districtId', 'name') // CHANGE HERE: Populate districtId
      .lean();
    ...
    return NextResponse.json({
      ...
      data: {
        ...populatedTaluka,
        districtName: (populatedTaluka?.districtId as any)?.name || 'Unknown District' // CHANGE HERE: districtId
      }
    });
  } catch (error: any) {
