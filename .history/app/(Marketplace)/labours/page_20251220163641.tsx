...
//       {/* Header Section */}
//       {
//       loading && labours.length ==0&&<div className="min-h-screen absolute w-full top-0 left-0 bg-[#fdfbfb8c]  z-[100] flex items-center justify-center">
...
//       <div
//   ref={tableRef}
//   className="hidden lg:block print:block bg-white rounded shadow"
// >
...
//     const tableColumn = ["Sr.", "Farmer", "Date", "Male", "Female", "Crop", "Work"];
//     const tableRows = labours.map((labour, index) => [
//       index + 1 + (page - 1) * 10,
//       labour.farmer.name,
//       new Date(labour.requiredDate).toLocaleDateString(),
...
//   const handlePrint = () => {
//     if (labours.length === 0) {
//       alert("No labour requests to print");
//       return;
//     }
...
//                     <div class="mobile-info">${labour.farmer.mobile}</div>
//                   </td>
...
//                 <td className="p-[.6rem] text-sm">{index + 1 + (page - 1) * 10}</td>
//                 <td className="p-[.6rem] text-sm">
//                   <div className="font-semibold">name - {labour?.farmer?.name}</div>
//                   <div className="text-gray-500 text-xs">phone - {labour?.farmer?.mobile}</div>
//                   <div className="text-gray-500 text-xs">address - {labour?.farmer?.address}</div>
//                   <div className="text-gray-500 text-xs">state - {labour?.farmer?.state}</div>
//                 </td>
//                 <td className="p-[.6rem] text-sm font-medium">
//                   {formatDate(labour.requiredDate)}
//                 </td>
...
//                   <div className="font-medium">{currentLabour?.farmer?.name}</div>
//                   <div className="text-sm text-gray-600">{currentLabour?.farmer?.mobile}</div>
//                   <div className="text-sm text-gray-600">{currentLabour?.farmer?.address}</div>
//                   <div className="text-sm text-gray-600">{currentLabour?.farmer?.state}</div>
//                 </div>
//                 <div className="bg-gray-50 p-[.6rem] text-sm rounded-lg">
//                   <div className="text-sm text-gray-500">Required Date</div>
//                   <div className="font-medium">{formatDate(currentLabour.requiredDate)}</div>
...
export default function LabourRequestsPage() {
  ...
  const handleExportExcel = () => {
    const data = labours.map((labour, index) => ({
      "Sr.": index + 1 + (page - 1) * 10,
      "Farmer Name": labour.farmer.name || "N/A",
      "Mobile": labour.farmer.mobile || "N/A",
      "Required Date": new Date(labour.requiredDate).toLocaleDateString(),
      "Male Workers": labour.male,
      "Female Workers": labour.female,
      "Total Workers": labour.male + labour.female,
        ...
        <table>
          ...
          <tbody>
            ${labours.map((labour, index) => {
              const requiredDate = new Date(labour.requiredDate).toLocaleDateString();
              const totalWorkers = labour.male + labour.female;
              const farmerName = labour.farmer.name || "N/A";
              const farmerMobile = labour.farmer.mobile || "N/A";
              
              return `
                <tr>
                  <td>${index + 1 + (page - 1) * 10}</td>
                  <td>
                    <div class="farmer-info">${farmerName}</div>
                    <div class="mobile-info">${farmerMobile}</div>
                  </td>
                  <td><strong>${requiredDate}</strong></td>
                  <td class="male-count">${labour.male}</td>
                  <td class="female-count">${labour.female}</td>
          ...
          <tbody className="divide-y divide-gray-100">
            {labours.map((labour, index) => (
              <tr key={labour._id} className="hover:bg-gray-50 transition-colors">
                <td className="p-[.6rem] text-sm">{index + 1 + (page - 1) * 10}</td>
                <td className="p-[.6rem] text-sm">
                  <div className="font-semibold">name - {labour?.farmer?.name || "N/A"}</div>
                  <div className="text-gray-500 text-xs">phone - {labour?.farmer?.mobile || "N/A"}</div>
                  <div className="text-gray-500 text-xs">address - {labour?.farmer?.address || "N/A"}</div>
                  <div className="text-gray-500 text-xs">state - {labour?.farmer?.state || "N/A"}</div>
                </td>
                <td className="p-[.6rem] text-sm font-medium">
                  {formatDate(labour.requiredDate)}
                </td>
        ...
        {labours.map((labour, index) => (
          <div key={labour._id} className="rounded p-[.6rem] text-sm border border-zinc-200 bg-white shadow">
            ...
            <div className="space-y-2">
              <div>
                <div className="text-sm text-gray-500">Farmer</div>
                <div className="font-medium">{labour?.farmer?.name || "N/A"}</div>
                <div className="text-xs text-gray-500">{labour?.farmer?.mobile || "N/A"}</div>
              </div>
              <div className="grid grid-cols-2 gap-[.6rem] text-sm">
                <div>
                  <div className="text-sm text-gray-500">Required Date</div>
          ...
          {currentLabour && (
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-[.6rem] text-sm rounded-lg">
                  <div className="text-sm text-gray-500">Farmer Details</div>
                  <div className="font-medium">{currentLabour?.farmer?.name || "N/A"}</div>
                  <div className="text-sm text-gray-600">{currentLabour?.farmer?.mobile || "N/A"}</div>
                  <div className="text-sm text-gray-600">{currentLabour?.farmer?.address || "N/A"}</div>
                  <div className="text-sm text-gray-600">{currentLabour?.farmer?.state || "N/A"}</div>
                </div>
                <div className="bg-gray-50 p-[.6rem] text-sm rounded-lg">
                  <div className="text-sm text-gray-500">Required Date</div>
                  <div className="font-medium">{formatDate(currentLabour.requiredDate)}</div>
      ...
      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        <Box sx={modalStyle}>
          <div className="text-center">
            ...
            <Typography className="text-gray-600 mb-6">
              Are you sure you want to delete the request from{" "}
              <span className="font-medium">{currentLabour?.farmer?.name || "Unknown"}</span> for{" "}
              <span className="font-medium">{currentLabour?.crop}</span>? 
              This action cannot be undone.
            </Typography>
            <div className="flex justify-center gap-[.6rem] text-sm">
