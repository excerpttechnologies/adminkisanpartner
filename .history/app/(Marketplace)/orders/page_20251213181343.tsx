...
export default function OrdersPage() {
  ...
  return (
    <div className="p-[.8rem] md:p-6  min-h-screen">
      {/* Header */}
      <div className="mb-8">
      ...
      {/* Filters Section */}
      <div className="bg-white rounded shadow p-[.8rem] md:p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-[.8rem] items-end">
          {/* Search Input */}
          ...
          {/* Action Buttons */}
          <div className="md:col-span-2 flex gap-[.8rem]">
            <button
      ...
      {/* Export Buttons */}
      <div className="flex flex-wrap gap-[.8rem] mb-6 bg-white p-[.8rem] rounded shadow">
        {[
          ...
          <button
            key={i}
            onClick={btn.onClick}
            className={`flex items-center gap-[.8rem] px-4 py-2 rounded transition-all duration-200 shadow-sm hover:shadow-md ${btn.color} font-medium`}
          >
      ...
      <div className="bg-white rounded shadow-lg overflow-hidden" ref={tableRef}>
          ...
          <table className="w-full">
            <thead className=" border-b border-zinc-200">
              <tr className="*:text-zinc-800">
                <th className="p-[.8rem] text-left  font-semibold">Sr.</th>
                <th className="p-[.8rem] text-left  font-semibold">Date</th>
                <th className="p-[.8rem] text-left  font-semibold">Crop Detail</th>
                <th className="p-[.8rem] text-left  font-semibold">Ordered By</th>
                <th className="p-[.8rem] text-left  font-semibold">Status</th>
                <th className="p-[.8rem] text-left  font-semibold">Delivery</th>
                <th className="p-[.8rem] text-left  font-semibold">Payment</th>
                <th className="p-[.8rem] text-left  font-semibold">Amount</th>
                <th className="p-[.8rem] text-left  font-semibold">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredOrders.map((order, i) => (
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  <td className="p-[.8rem]">{i + 1}</td>
                  <td className="p-[.8rem] font-medium">{order.date}</td>
                  <td className="p-[.8rem]">
                    <div className="font-semibold text-gray-500">{order.crop}</div>
                  </td>
                  <td className="p-[.8rem]">{order.orderedBy}</td>
                  <td className="p-[.8rem]">
                  ...
                  <td className="p-[.8rem] text-sm text-gray-700">{order.delivery}</td>
                  <td className="p-[.8rem]">
                    <span className="px-2 py-1 bg-gray-100  rounded text-sm">
                      {order.payment}
                    </span>
                  </td>
                  <td className="p-[.8rem] font-bold text-green-700">{order.amount}</td>
                  <td className="p-[.8rem]">
                    <div className="flex gap-[.8rem]">
                      <button
                        onClick={() => handleViewClick(order)}
                        className="p-[.8rem] text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                        title="View Details"
                      >
                        <FaEye />
                      </button>
                      <button
                        ...
                        className="p-[.8rem] text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                        title="Edit Order"
                      >
                        <FaEdit />
                      </button>
                      <button
                        ...
                        className="p-[.8rem] text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                        title="Delete Order"
                      >
                        <FaTrash />
                      </button>
        ...
        {/* Mobile Cards */}
        <div className="lg:hidden  space-y-4 p-[.8rem]">
          {filteredOrders.map((order, i) => (
            <div key={order.id} className=" rounded p-[.8rem] border border-zinc-200 shadow">
      ...
      {/* Pagination */}
      {filteredOrders.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center mt-8 gap-[.8rem]">
      ...
      >
        <Fade in={editModalOpen}>
          <Box sx={modalStyle}>
            ...
            <div className="flex justify-end gap-[.8rem] mt-6">
              <Button
            ...
            {currentOrder && (
              <>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-[.8rem]">
                    <div className="bg-gray-50 p-[.8rem] rounded-lg">
                      <div className="text-sm text-gray-500">Date</div>
