...
// For table display purposes - create a simplified interface
interface DisplayOrder {
  ...
  delivery: string;
  payment: string;
  amount: string; // You might need to add this to your actual Order interface
}
...
export default function OrdersPage() {
  ...
  const convertToDisplayOrders = (apiOrders: Order[]): DisplayOrder[] => {
    return apiOrders.map(order => ({
      id: order.orderId || order._id,
      date: new Date(order.delivery.date).toISOString().split('T')[0],
      crop: order.cropDetail.cropName,
      orderedBy: order.farmer,
      status: order.status.admin, // Using admin status for display
      delivery: `${order.delivery.date} ${order.delivery.time}`,
      payment: order.payment.status,
      amount: "$0" // You'll need to add amount to your Order interface
    }));
  };
  ...
  const handleEditClick = (order: DisplayOrder) => {
  ...
  const handleSaveEdit = () => {
    // Here you would update the order via API
    console.log("Saving order:", editForm);
    
    // Update display orders
    setDisplayOrders(prev => prev.map(order => 
      order.id === currentOrder.id ? { ...order, ...editForm } : order
    ));
  ...
  const handleDeleteClick = (order: DisplayOrder) => {
    setCurrentOrder(order);
  ...
  const handleConfirmDelete = async () => {
    try {
      // Delete from API
      await axios.delete(`/api/order/${currentOrder.id}`);
      
      // Update local state
      setDisplayOrders(prev => prev.filter(order => order.id !== currentOrder.id));
  ...
  // View order details
  const handleViewClick = (order: DisplayOrder) => {
    setCurrentOrder(order);
  ...
  const getOrders = async () => {
    try {
      ...
      if (Array.isArray(apiOrders)) {
        ...
        const displayData = convertToDisplayOrders(apiOrders);
        setDisplayOrders(displayData);
      } else {
    ...
    <div className="p-[.6rem] text-black text-sm md:p-1 overflow-x-auto  min-h-screen">
      ...
      <div className="mb-8 flex justify-between">
        ...
        <button 
          ...
          className="bg-green-500 text-white w-32 h-12 rounded hover:bg-green-600 transition-colors"
        >
          Add new
        </button>
      </div>
                ...
                <div className="grid grid-cols-2 gap-[.6rem] text-sm">
                  <div>
                    ...
                    <div className={`text-sm font-medium
                      ${order.payment === "Completed" ? "text-green-700" :
                        order.payment === "Failed" ? "text-red-700" :
                        "text-yellow-700"}`}>
                      {order.payment}
                    </div>
                  </div>
                    ...
                    <div className="bg-gray-50 p-[.6rem] text-sm rounded-lg">
                      ...
                      <div>
                        <span className={`px-2 py-1 rounded text-xs font-medium
                          ${currentOrder.status === "Approved" ? "bg-green-100 text-green-700" :
                            currentOrder.status === "Rejected" ? "bg-red-100 text-red-700" :
                  ...
                  <div className="grid grid-cols-2 gap-[.6rem] text-sm">
                    <div className="bg-gray-50 p-[.6rem] text-sm rounded-lg">
                      ...
                      <div className={`font-medium
                        ${currentOrder.payment === "Completed" ? "text-green-700" :
                          currentOrder.payment === "Failed" ? "text-red-700" :
                          "text-yellow-700"}`}>
