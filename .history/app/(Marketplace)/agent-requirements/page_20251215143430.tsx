...
export interface Requirement {
  ...
  qualities: Quality[];
  requirementDate: string; // ISO date string
  location: string;
...
export default function RequirementsPage() {
  ...
  const handleDelete = () => {
    if (selected) {
      setRows(rows.filter(row => row._id !== selected._id));
      setDeleteOpen(false);
  ...
  return (
    <div className="min-h-screen text-black    md:p-1">
      {/* Header */}
      <div className="mb-5">
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={exportExcel}
                  className="p-2.5 border border-gray-300 bg-[#B9F8CF]  rounded-lg hover:bg-gray-50 transition-colors group relative"
                  title="Export to Excel"
                >
                  <FaFileExcel className="w-5 h-5 text-green-600 group-hover:text-gray-900" />
                </button>
                <button
                  onClick={exportCSV}
                  className="p-2.5 border border-gray-300 bg-[#BEDBFF] rounded-lg hover:bg-gray-50 transition-colors group relative"
                  title="Export to CSV"
                >
                  <FaFileCsv className="w-5 h-5 text-blue-600 group-hover:text-gray-900" />
                </button>
                <button
                  onClick={exportPDF}
                  className="p-2.5 border border-gray-300 bg-[#F5C9C9] rounded-lg hover:bg-gray-50 transition-colors group relative"
                  title="Export to PDF"
                >
                  <FaFilePdf className="w-5 h-5 text-red-600 group-hover:text-gray-900" />
                </button>
                <button
                  onClick={printTable}
                  className="p-2.5 border border-gray-300 bg-[#EAD5FF] rounded-lg hover:bg-gray-50 transition-colors group relative"
                  title="Print"
                >
                  <FaPrint className="w-5 h-5 text-violet-600 group-hover:text-gray-900" />
                </button>
              </div>
            </div>
          </div>
...
<div className="relative">
  ...
  <div className="hidden md:block overflow-x-auto">
    <table ref={tableRef} className="min-w-full">
      <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
        <tr>
          {["Date","Category","Item","Seed","Quantity","Quality","Posted By","Status","Actions"].map(h => (
            <th
              key={h}
              className="px-6 py-2 text-left text-xs font-semibold text-gray-700 uppercase border-b"
            >
              {h}
            </th>
          ))}
      ...
      <tbody className="divide-y">
        {paginatedRows.map(row => (
          <tr key={row.id} className="hover:bg-gray-50">
            ...
            <td className="px-6 py-2 text-xs font-semibold">{row.item}</td>
            <td className="px-6 py-2 text-xs">{row.seed}</td>
            <td className="px-6 py-2 text-xs">{row.quantity}</td>
            <td className="px-6 py-2 text-xs">{row.quality}</td>

            <td className="px-6 py-2 flex items-center text-xs gap-2">
            ...
            <td className="px-6 py-2 text-xs">
              <span className={`px-3 py-1 text-xs rounded-full border ${getStatusColor(row.status)}`}>
                {row.status}
              </span>
            </td>
  ...
  <div className="md:hidden space-y-4 p-4">
    {paginatedRows.map(row => (
      <div key={row.id} className="border rounded-xl shadow-sm p-4 space-y-2">
        ...
        <div className="flex justify-between">
          ...
          <span className={`px-2 py-1 text-xs rounded-full border ${getStatusColor(row.status)}`}>
            {row.status}
          </span>
        </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button
            onClick={() => { setSelected(row); setDeleteOpen(true); }}
            className="flex-1 py-2 bg-rose-50 text-rose-600 rounded-lg"
          >
            Delete
          </button>
        </div>
