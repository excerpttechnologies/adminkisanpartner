...
import { getDashboardData } from "../config/dashboarddata";
  ...
  const fetchDashboardCards = async () => {
    try {
      ...
      setDashboard((prev: any) => ({
        ...
        // Cards
        totalFarmers: res.stats.totalFarmers,
        totalAgents: res.stats.totalAgents,

        totalTransports: res.stats.totalTransports,
        activeTransports: res.stats.activeTransports,

        totalOrders: res.stats.totalOrders,
        pendingOrders: res.stats.pendingOrders,
        completedOrders: res.stats.completedOrders,

        labourRequests: res.stats.totalLabours,
        activePostings: res.stats.activePostings,
  ...
  const fetchDashboardCharts = async () => {
    try {
      ...
      setDashboard((prev: any) => ({
        ...
        // Charts
        monthlyCropPostings: chartData.monthlyCropPostings || [],
        orderTrends: chartData.orderTrends || [],
  ...
  return (
    <div className="p-2 bg-gray-50 min-h-[90vh] space-y-3 xl:w-[83vw] lg:w-[75vw]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <DashboardCard
          title="Labour Requests"
          value={dashboard.labourRequests || 0}
          subtitle="Pending farm labour requests"
          icon={<FaHandsHelping className="text-red-500" />}
        />
        ...
        <DashboardCard
          title="Active Postings"
          value={dashboard.activePostings || 0}
          subtitle="Current crop listings"
          icon={<FaBullhorn className="text-yellow-500" />}
        />
      ...
      {/* ===== CHARTS ===== */}
     {
      user?.role=="admin"&& <div className="flex w-full lg:flex-row flex-col gap-3">
        <div className="lg:w-[55vw] w-full">
          <BarChartBox
            data={dashboard.monthlyCropPostings || []}
            year={dashboard.year}
          />
        </div>

        <div className="flex flex-col lg:w-[30vw] gap-2">
          <LineChartBox
