...
export default function FarmerAcceptReport() {
  ...
  const [uniqueTaluks, setUniqueTaluks] = useState<string[]>([]);
  ...
  }, [pagination.currentPage, limit, user]);
  ...
  const fetchReportData = async (page: number, limit: number): Promise<void> => {
    try {
      ...
      // Add role-based filtering parameter
      if (user?.role === 'subadmin' && user?.taluka) {
        params.append('taluka', user.taluka);
        params.append('userRole', 'subadmin');
      ...
      if (data.success) {
        const filteredData = filterDataByUserRole(data.data);
        setReportData(filteredData);
        
        // Extract unique taluks for admin filter display
        if (user?.role === 'admin') {
          const allTaluks = extractAllTaluks(data.data);
          setUniqueTaluks(allTaluks);
        }
  ...
  const filterDataByUserRole = (data: ReportData[]): ReportData[] => {
    if (!user) return data;
    
    if (user.role === 'subadmin' && user.taluka) {
      // Filter data where either farmer or trader is in subadmin's taluka
      return data.filter(item => 
        item.farmerTaluk === user.taluka || item.traderTaluk === user.taluka
      );
    }
  ...
  const extractAllTaluks = (data: ReportData[]): string[] => {
    const taluks = new Set<string>();
    data.forEach(item => {
      if (item.farmerTaluk) taluks.add(item.farmerTaluk);
      if (item.traderTaluk) taluks.add(item.traderTaluk);
    });
  ...
  const LocationInfo = ({ 
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-gray-900 capitalize">{type} Location</p>
      </div>
      <div className="flex flex-wrap gap-2">
        ...
        {district && (
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-green-50 text-green-800 border border-green-200">
            {district}
          </span>
        )}
        {taluk && (
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Accepted Offers Report</h1>
                  <p className="text-gray-600 mt-1">
                    Overview of farmer accepted offers and purchases with location details
                  </p>
                  ...
                  {user && (
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      {user.role === 'admin' && uniqueTaluks.length > 0 && (
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">
                    {user.role === 'admin' ? 'Administrator View' : 'Sub-Administrator View'}
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    {user.role === 'admin' 
                      ? 'You are viewing data from all taluks.'
                      : `You are viewing data only from ${user.taluka} taluka.`
                    }
                  </p>
                </div>
              </div>
              ...
              {user.role === 'admin' && uniqueTaluks.length > 0 && (
                <div className="flex-1 md:text-right">
                  <p className="text-sm font-medium text-gray-700">Available Taluks: {uniqueTaluks.length}</p>
                  <div className="flex flex-wrap gap-1 justify-end mt-2">
                    {uniqueTaluks.slice(0, 5).map(taluk => (
                      <span key={taluk} className="px-2 py-1 text-xs bg-white text-gray-700 rounded border border-gray-300">
                        {taluk}
                      </span>
                    ))}
                    {uniqueTaluks.length > 5 && (
                      <span className="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded">
                        +{uniqueTaluks.length - 5} more
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className={`p-4 rounded-lg mb-6 border ${
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {user.role === 'admin' 
                        ? 'You can view data from all taluks. Filters will be applied to the entire dataset.'
                        : `You can only view data from ${user.taluka} taluka. Filters will be applied within your assigned taluka.`
                      }
                    </p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-sm p-5 border border-gray-200">
            ...
            <p className="text-xs text-gray-500 mt-3">
              {user?.role === 'subadmin' ? `In ${user.taluka} taluka` : 'Across all taluks'}
            </p>
          </div>
                  ...
                  >
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="mt-3">
                            <LocationInfo 
                              type="trader"
                              state={item.traderState}
                              district={item.traderDistrict}
                              taluk={item.traderTaluk}
                            />
                          </div>
                        </div>
                    ...
                    <td className="px-6 py-5">
                      <div className="space-y-3">
                        ...
                        <div className="pt-3 border-t border-gray-100">
                          <p className="text-xs font-medium text-gray-700 mb-2">Location Match</p>
                          <div className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${
                            item.farmerState === item.traderState && item.farmerDistrict === item.traderDistrict
                        ...
                        {user && (
                          <div className="pt-3 border-t border-gray-100">
                            <p className="text-xs font-medium text-gray-700 mb-2">Access Level</p>
                            <div className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium ${
                              user.role === 'admin'
                              ...
                              {user.role === 'admin' 
                                ? 'Admin View: All Taluks' 
                                : `Subadmin View: ${user.taluka} Taluka`
                              }
                            </div>
                          </div>
                        )}
            ...
            >
              ...
              <div className="mb-5 grid grid-cols-2 gap-3">
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                  <div className="flex items-center gap-2 mb-2">
                    <HomeIcon className="h-4 w-4 text-blue-600" />
                    <p className="text-xs font-semibold text-gray-900">Farmer Location</p>
                  </div>
                  {item.farmerState ? (
                    <>
                      <p className="text-sm text-gray-900">{item.farmerState}</p>
                      <p className="text-xs text-gray-600">{item.farmerDistrict}, {item.farmerTaluk}</p>
                    </>
                  ) : (
                ...
                <div className="bg-purple-50 rounded-lg p-3 border border-purple-100">
                  <div className="flex items-center gap-2 mb-2">
                    <BuildingOfficeIcon className="h-4 w-4 text-purple-600" />
                    <p className="text-xs font-semibold text-gray-900">Trader Location</p>
                  </div>
                  {item.traderState ? (
                    <>
                      <p className="text-sm text-gray-900">{item.traderState}</p>
                      <p className="text-xs text-gray-600">{item.traderDistrict}, {item.traderTaluk}</p>
                    </>
                  ) : (
                    <p className="text-xs text-gray-500">Location not available</p>
                  )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900">
                        {user.role === 'admin' ? 'Admin View' : 'Subadmin View'}
                      </p>
                      <p className="text-xs text-gray-600">
                        {user.role === 'admin' 
                          ? 'Viewing all taluks' 
                          : `Viewing ${user.taluka} taluka only`
                        }
                      </p>
                    </div>
                  </div>
        ...
        {reportData.length === 0 && !loading && (
          <div className="py-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {user?.role === 'subadmin' 
                ? `No accepted offers found in ${user.taluka} taluka`
            ...
            <p className="text-gray-600 max-w-md mx-auto mb-6">
              {Object.values(filters).some(f => f !== '' && f !== 'all')
                ? 'No results match your current filters. Try adjusting your search criteria.'
                : user?.role === 'subadmin'
                ? `There are no accepted offers in ${user.taluka} taluka at the moment.`
                : 'There are no accepted offers at the moment.'
              }
            </p>
            {Object.values(filters).some(f => f !== '' && f !== 'all') && (
              <button
              ...
              <div className="text-sm text-gray-700 bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
                ...
                {user?.role === 'subadmin' && (
                  <span className="ml-2 text-xs text-purple-600 font-medium">
                    (Filtered by {user.taluka} taluka)
                  </span>
                )}
              </div>
