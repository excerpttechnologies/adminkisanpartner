...
import connectDB from '@/app/lib/Db';
import Product from '@/app/models/Product';
import Farmer from '@/app/models/Farmer'; // This model handles both farmers and traders
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pipeline: any[] = [
      ...
      {
        $lookup: {
          from: 'farmers', // Both farmers and traders are in this collection
          localField: 'farmerId',
          foreignField: 'farmerId', // Note: farmers use farmerId field
          as: 'farmerDetails'
        }
      },
      ...
      // Lookup trader details from the same farmers collection
      {
        $lookup: {
          from: 'farmers', // Same collection for traders too
          localField: 'gradePrices.purchaseHistory.traderId',
          foreignField: 'traderId', // Traders use traderId field
          as: 'traderDetails'
        }
      },
      
      // Unwind trader details
      { $unwind: { path: '$traderDetails', preserveNullAndEmptyArrays: true } },
      ...
      // Project only the fields we need
      {
        $project: {
          ...
          farmerState: '$farmerDetails.personalInfo.state',
          farmerDistrict: '$farmerDetails.personalInfo.district',
          farmerTaluk: '$farmerDetails.personalInfo.taluk',
          
          // Trader location details - from the same collection
          traderState: '$traderDetails.personalInfo.state',
          traderDistrict: '$traderDetails.personalInfo.district',
          traderTaluk: '$traderDetails.personalInfo.taluk'
        }
      },
