...
//   const generateRandomDistance = useCallback(() => {
//     const distances = [15, 18, 23, 28, 32, 37, 42, 48, 53, 58, 62, 67, 72, 78, 80];
//     const randomIndex = Math.floor(Math.random() * distances.length);
//     return `${distances[randomIndex]}`;
//   }, []);
...
//     if (!transporterId || transporterId === 'undefined' || transporterId.startsWith('test-')) {
//       return '';
//     }
...
//       }
//     } catch (error: any) {
//       // console.error(`Error fetching transporter taluk:`, error.message);
//     }
//     return '';
//   };
...
//     try {
//       const endpoints = [
//         `/api/farmers?search=${traderId}`,
//         `${API_BASE}/api/farmers/${traderId}?role=trader`,
//         `${API_BASE}/farmer/register/all?traderId=${traderId}&role=trader`
//       ];
...
//             const traderData = Array.isArray(response.data.data) ? response.data.data[0] : response.data.data;
            
//             if (traderData) {
//               const traderDetails = {
//                 traderName: traderData.personalInfo?.name || traderData.name || 'Trader',
//                 traderMobile: traderData.personalInfo?.mobileNo || traderData.mobile || 'N/A',
//                 location: {
//                   address: traderData.personalInfo?.address || traderData.address || "",
//                   state: traderData.personalInfo?.state || traderData.state || "",
//                   pincode: traderData.personalInfo?.pincode || traderData.pincode || "",
//                   district: traderData.personalInfo?.district || traderData.district || "",
//                   taluk: traderData.personalInfo?.taluk || traderData.taluk || "",
//                   villageGramaPanchayat: traderData.personalInfo?.villageGramaPanchayat || traderData.village || "",
//                 }
//               };
...
//           exactAddress: market.address || market.exactAddress || "",
...
//             let pickupMarket = null;
//             if (orderDetails?.productItems?.[0]?.nearestMarket) {
//               pickupMarket = await fetchMarketDetails(orderDetails.productItems[0].nearestMarket);
//             }
            
//             let traderDetails = null;
//             if (order.traderId) {
//               traderDetails = await fetchTraderDetails(order.traderId);
//             }
...
//               ...order,
//               productItems: orderDetails?.productItems || [],
//               pickupMarket: pickupMarket || undefined,
//               traderDetails: traderDetails || {
//                 traderName: order.traderName || 'Trader',
//                 traderMobile: order.traderMobile || 'N/A',
//                 location: {
//                   address: '',
//                   district: '',
//                   state: '',
...
//             };
//           } catch (err) {
//             console.error(`Error enriching order ${order.orderId}:`, err);
//             return {
//               ...order,
//               productItems: [],
//               traderDetails: {
...
//   const getDeliveryDistrict = (order: Order) => {
//     const district = order.traderDetails?.location?.district;
//     const taluk = order.traderDetails?.location?.taluk;
...
//                       </div>
//                       <div style={{ color: '#666' }}>
//                         {order.pickupMarket?.district || 'Location not specified'}
...
//                     <div>
//                       <div style={{ fontWeight: 'bold' }}>
//                         {order.traderDetails?.traderName || order.traderName || 'Trader'}
//                       </div>
//                       <div style={{ color: '#666' }}>
...
//                         </div>
//                       ) : (
//                         <div style={{ color: '#999', fontStyle: 'italic' }}>Location not specified</div>
//                       )}
//                     </td>
                    
//                     <td style={{ padding: 16, verticalAlign: 'top' }}>
//                       <div>
//                         <div><strong>{order.traderDetails?.traderName || order.traderName || 'Trader'}</strong></div>
//                         <div style={{ fontSize: '12px', color: '#666', marginTop: 4 }}>
//                           {getDeliveryDistrict(order)}
//                         </div>
//                       </div>
...
const AdminMarkettoTrader: React.FC = () => {
  ...
  // Calculate total pages when filteredOrders changes
  useEffect(() => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      setOrders(enrichedOrders);
      setCurrentPage(1); // Reset to first page when orders are fetched
    } catch (error) {
      console.error('Error fetching orders:', error);
      setOrders([]);
    } finally {
  ...
  return (
    <div style={{ padding: isMobile ? 12 : 20, maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          {/* <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 14, color: '#495057' }}>Show:</span>
              <select
                value={itemsPerPage}
              ...
              <span style={{ fontSize: 14, color: '#495057' }}>per page</span>
            </div>
          </div> */}
        </div>
      )}
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div style={{ overflowX: 'auto', border: '1px solid #e0e0e0', borderRadius: 8, background: '#fff', marginBottom: 24 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f5f5f5' }}>
                <th style={{ padding: 16, textAlign: 'left', borderBottom: '1px solid #e0e0e0', fontWeight: 'bold', color: '#333' }}>Order ID</th>
      ...
      {filteredOrders.length > 0 && (
        <div style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 14, color: '#495057' }}>per page</span>
            <span style={{ fontSize: 14, color: '#6c757d', marginLeft: 12 }}>
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button
                ...
                style={{
                  padding: '8px 12px',
                  background: currentPage === totalPages ? '#e9ecef' : '#6c757d',
