...
//       const actorName = extractAdminName(log).toLowerCase();
//       if (actorName.includes(query)) return true;

//       // Search in actor ID
//       const actorId = getFullActorId(log.actorId).toLowerCase();
//       if (actorId.includes(query)) return true;

//       // Search in action
//       if (log.action?.toLowerCase().includes(query)) return true;
...
//       // Search in log ID
//       const logId = (log._id || log.id || "").toString().toLowerCase();
//       if (logId.includes(query)) return true;

//       // Search in IP address
//       if (log.ipAddress?.toLowerCase().includes(query)) return true;

//       // Search in actor role
//       if (log.actorRole?.toLowerCase().includes(query)) return true;

//       // Search in changes object
//       if (log.changes) {
//         const changesString = JSON.stringify(log.changes).toLowerCase();
//         if (changesString.includes(query)) return true;
//       }
...
//     return { start, end, total: filteredLogs.length };
//   };
...
//                   {searchQuery ? `Search: "${searchQuery}"` : "No search applied"}
//                 </p>
//               </div>
//               <div className="p-3 bg-yellow-50 rounded-lg">
//                 <Folder className="w-6 h-6 text-yellow-600" />
...
//             placeholder="Search audit logs by user, action, module, or ID..."
//             className="w-full pl-12 pr-10 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
//           />
//           {searchQuery && (
//             <button
//               onClick={clearSearch}
...
//             Found {filteredLogs.length} audit logs matching "{searchQuery}"
//           </div>
//         )}
//       </div>
...
//               {searchQuery 
//                 ? `No audit logs found for "${searchQuery}". Try a different search term.`
...
//               <div className="text-sm text-gray-600">
//                 Showing <span className="font-semibold">{start}</span> to <span className="font-semibold">{end}</span> of <span className="font-semibold">{total}</span> audit logs
//                 {searchQuery && ` (filtered from ${logs.length} total logs)`}
//               </div>
//             </div>
...
//         await axios.delete("/api/audit-logs", {
//           data: { logId: logToDelete }
//         });
        
//         // Remove from local state
//         setLogs(logs.filter(log => log._id !== logToDelete));
        
//       } else if (deleteMode === 'multiple' && selectedLogs.size > 0) {
...
//         // Remove from local state
//         setLogs(logs.filter(log => !selectedLogs.has(log._id)));
//         setSelectedLogs(new Set());
...
//       setSelectedLogs(new Set());
//     } else {
//       const allLogIds = new Set(paginatedLogs.map(log => log._id));
...
//                   {deleteMode === 'single' ? 'Delete Log' : 
//                    deleteMode === 'multiple' ? 'Delete Selected Logs' : 
//                    'Delete All Logs'}
//                 </h3>
//               </div>
              
//               <p className="text-gray-600 mb-6">
//                 {deleteMode === 'single' ? 'Are you sure you want to delete this audit log? This action cannot be undone.' :
//                  deleteMode === 'multiple' ? `Are you sure you want to delete ${selectedLogs.size} selected audit logs? This action cannot be undone.` :
...
//           <div className="flex gap-3">
//             {/* Delete Selected Button (only show if logs are selected) */}
//             {selectedLogs.size > 0 && (
//               <button 
...
//                   Select all on this page ({paginatedLogs.length} logs)
//                 </label>
//               </div>
//             )}
...
//                         <button
//                           onClick={() => handleDeleteSingle(logId)}
//                           className="px-3 py-2 text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0"
...
export default function AuditLogsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      {/* Delete Confirmation Modal - Fixed with better z-index and positioning */}
      {showDeleteModal && (
        <div className="fixed inset-0 bg-[#56565662] bg-opacity-50 flex items-center justify-center z-[9999] p-4">
