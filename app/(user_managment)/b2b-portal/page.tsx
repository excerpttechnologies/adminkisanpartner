...
//   try {
//     const res = await fetch('https://kisan.etpl.ai/category/all?page=1&limit=100');
//     if (!res.ok) return [];
...
// const fetchSubCategoriesByCategoryId = async (categoryId: string): Promise<SubCategory[]> => {
//   try {
//     const res = await fetch('https://kisan.etpl.ai/subcategory/all?page=1&limit=100');
//     if (!res.ok) return [];
//     const data = await res.json();
//     const all: SubCategory[] = Array.isArray(data) ? data : (data.data || []);
...
//                         {product.subCategory && (
//                           <p className="text-xs text-blue-500 mb-2">↳ {product.subCategory}</p>
//                         )}
//                         <p className="text-sm text-gray-600 mb-3 line-clamp-2">{product.description}</p>
//                         <div className="flex justify-between items-center mb-3">
//                           <span className="text-xl font-bold text-blue-600">₹{product.price.toLocaleString()}</span>
...
//       // If editing and category already set, load its subcategories
//       if (product?.category) {
//         const matched = data.find(
//           (c: Category) => c.categoryName === product.category || c._id === product.category
//         );
//         if (matched) {
//           setFormData((prev) => ({ ...prev, category: matched._id }));
//           loadSubCategories(matched._id);
//         }
//       }
//     };
//     loadCategories();
...
//     // Resolve category name for saving
//     const selectedCat = categories.find((c) => c._id === formData.category);
//     const selectedSubCat = subCategories.find((s) => s._id === formData.subCategory);

//     onSave({
//       ...formData,
//       images: filledImages,
//       category: selectedCat?.categoryName || formData.category,
//       subCategory: selectedSubCat?.subCategoryName || formData.subCategory,
//     });
//   };
...
//             <label className="block text-sm font-medium mb-1">Category *</label>
//             {loadingCategories ? (
//               <div className="w-full px-3 py-2 border rounded-lg bg-gray-50 text-gray-400 text-sm flex items-center gap-2">
...
//           {/* Sub Category — shown only after category is selected */}
//           {formData.category && (
//             <div>
//               <label className="block text-sm font-medium mb-1">
//                 Sub Category
...
//                 <div className="w-full px-3 py-2 border rounded-lg bg-gray-50 text-gray-400 text-sm">
...
//                     </span>
//                     {formData.images[index] && (
//                       <button type="button" onClick={() => clearImage(index)} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1">
//                         <X className="w-3 h-3" /> Clear
//                       </button>
//                     )}
//                   </div>
