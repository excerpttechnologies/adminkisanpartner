...
export default function CropManagementPage() {
  ...
  const copyData = () => {
    ...
    try {
      ...
      // Create table rows with safe handling
      ...
      // Calculate column widths with safe handling
      const columnWidths = headers.map((header, colIndex) => {
        const maxHeaderLength = header.length;
        // Safely get max data length
        const maxDataLength = rows.reduce((max, row) => {
          const cell = row[colIndex] || "";
          const cellLength = cell.toString().length;
          return Math.max(max, cellLength);
        }, 0);
        return Math.max(maxHeaderLength, maxDataLength, 10); // Minimum width of 10
      });
