...
//   useEffect(() => {
//     getOrders();
//   }, [search, status, date, page,pathname,limit]);
...
//       <div className="lg:hidden flex  flex-wrap gap-[.6rem] text-sm bg-white p-[.6rem] text-sm  shadow">
//         {[
...
//       {/* Filters Section */}
//       <div className="bg-white rounded lg:rounded-none shadow p-[.4rem] text-sm  ">
...
//              {/* <select value={limit} onChange={(e)=>setLimit(Number(e.target.value))} className="p-2 border border-zinc-300 rounded">
//               {
...
//             </button>
//           </div>
//           <div className="lg:flex hidden  ml-auto flex-wrap gap-[.6rem] text-sm  bg-white p-[.6rem] text-sm  ">
//         {[
//           { label: "Copy", icon: FaCopy, onClick: handleCopyToClipboard, color: "bg-gray-100 hover:bg-gray-200 text-gray-800" },
//           { label: "Excel", icon: FaFileExcel, onClick: handleExportExcel, color: "bg-green-100 hover:bg-green-200 text-green-800" },
//           { label: "CSV", icon: FaFileCsv, onClick: handleExportCSV, color: "bg-blue-100 hover:bg-blue-200 text-blue-800" },
...
//                   {typeof order.farmer === "string" ? order.farmer : order.farmer?.name}
//                 </td>
//                 <td className="p-[.6rem] flex flex-col text-sm">
//                   <span className={`px-3  rounded-full text-xs font-semibold
//                     ${order.status.admin === "Approved" ? " text-green-800" :
//                       order.status.admin === "Rejected" ? " text-red-800" :
//                       "bg-yellow-100 text-yellow-800"}`}>
//                     admin - {order.status.admin}
//                   </span>
//                    <span className={`px-3  rounded-full text-xs font-semibold
//                     ${order.status.farmer === "Approved" ? " text-green-800" :
//                       order.status.farmer === "Rejected" ? " text-red-800" :
//                       "bg-yellow-100 text-yellow-800"}`}>
//                     farmer - {order.status.farmer}
//                   </span>
//                 </td>
    ...
    <div className="p-[.6rem] relative text-black text-sm md:p-1 overflow-x-auto min-h-screen">
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-wrap  gap-y-2">
                  ...
                  <span className={`ml-3 px-2 py-1 rounded-full text-xs font-semibold
                    ${order.status.farmer === "Approved" ? "bg-green-100 text-green-800" :
                      order.status.farmer === "Rejected" ? "bg-red-100 text-red-800" :
                      "bg-yellow-100 text-yellow-800"}`}>
                   farmer - {order.status.farmer}
                  </span>
                </div>
      ...
      {orders.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold mb-2">No orders found</h3>
          <p className="text-gray-500">Try adjusting your search or filters</p>
          <button onClick={getOrders} className="border border-green-300 text-sm rounded p-1 mt-2 px-3 cursor-pointer bg-green-300 text-white">reload</button>
        </div>
      )}
                    ...
                    <div className="flex flex-col gap-y-2">
                      <span className={`px-2 py-1 rounded text-xs font-medium
                        ${currentOrder.status.admin === "Approved" ? "bg-green-100 text-green-700" :
                          currentOrder.status.admin === "Rejected" ? "bg-red-100 text-red-700" :
                          "bg-yellow-100 text-yellow-700"}`}>
                      admin -  {currentOrder.status.admin}
                      </span>
                      <span className={`px-2 py-1 rounded text-xs font-medium
                        ${currentOrder.status.farmer === "Approved" ? "bg-green-100 text-green-700" :
                          currentOrder.status.farmer === "Rejected" ? "bg-red-100 text-red-700" :
                          "bg-yellow-100 text-yellow-700"}`}>
                      farmer -  {currentOrder.status.farmer}
                      </span>
                    </div>
                  </div>
                </div>
